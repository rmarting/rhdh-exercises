...
https://docs.redhat.com/en/documentation/red_hat_developer_hub/{rhdh_version}/html/interacting_with_red_hat_developer_lightspeed_for_red_hat_developer_hub/index[Red Hat Developer Lightspeed^]
...
For detailed information, refer to the https://docs.redhat.com/en/documentation/red_hat_developer_hub/{rhdh_version}/html-single/interacting_with_red_hat_developer_lightspeed_for_red_hat_developer_hub/index[official documentation^].

== Overview

The Lightspeed integration consists of several components:

- **Lightspeed Core Service (LCS)**: The backend service that handles AI interactions, including
the LLM (Large Language Model) capabilities for processing queries.
...
[source,bash,subs="+macros,+attributes"]
----
oc apply -f ./rhdh-lightspeed-auth-secrets.yaml -n {rhdh_namespace}
----
...
the necessary sidecar containers. This is very easy, just enabling this flavour in the Backstage Custom Resource:
...
This enabled automatically to add set of new init containers and side container to create
...
* **TechDocs Queries**:
  ...
  - "Fetch page `index.html` from techdoc with reference `component:default/my-sample-app`"
...
* How to configure LLM secrets and Lightspeed configmaps
